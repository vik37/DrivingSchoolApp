...
namespace DrivingSchoolApp.Services.CategoryServices.BCategoryAPI.DataAccess.EntityFrameworkConfigurations
{
	public static class EntityFrameworkDbInstructorConfigurations
	{
		public static void InstructorConfigurations(ModelBuilder modelBuilder)
		{
			...
			modelBuilder.Entity<Instructor>().Property(x => x.TotalWorkExperience).HasColumnName("TotalWorkExperience").HasColumnType("int").IsRequired(true);
			modelBuilder.Entity<Instructor>().Property(x => x.DriveExperience).HasColumnName("DriveExperience").HasColumnType("int").IsRequired(true);
